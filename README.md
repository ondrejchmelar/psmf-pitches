...
./.venv/bin/python scrape_tables.py          # final standings -> data/tables/
...
every season back to 2007 but links a team to none of its earlier selves and has
no way to ask where a team played in a given year, so the first pass writes that
index down — every season's division pages, about 3,900 requests — and the second
reads each team's page for every season it appears in. What it is after is the
referee's write-up: every played match has a paragraph describing it, and it
exists nowhere else. One file per team under `data/hist`, so an interrupted run
