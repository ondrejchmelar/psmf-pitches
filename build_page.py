...
.sortby { font:inherit; color:inherit; letter-spacing:inherit; text-transform:inherit;
  background:none; border:0; padding:0; cursor:pointer; white-space:nowrap; }
.sortby:hover { color:var(--ink); }
.span { font:inherit; color:inherit; background:none; border:0; padding:0; cursor:pointer;
  white-space:nowrap; border-bottom:1px solid transparent; }
.span:hover { color:var(--accent); border-bottom-color:currentColor; }
.sortby[aria-sort] { color:var(--ink); }
...
.glove { color:var(--muted); flex:none; }
.capt { display:inline-flex; align-items:center; justify-content:center; flex:none;
  width:16px; height:16px; border-radius:50%; border:1px solid currentColor;
  color:var(--faint); font:700 10px/1 ui-monospace,SFMono-Regular,Menlo,monospace; }
...
// A goalkeeper's glove: three fingers, a palm and a thumb. Drawn rather than
// written because the word was as wide as some of the names it followed.
const GLOVE = `<svg class="glove" viewBox="0 0 24 24" width="16" height="16"
    role="img" aria-label="gólman"><title>gólman</title>
    <path fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round"
      stroke-linejoin="round" d="M8.2 11V6.2a1.5 1.5 0 0 1 3 0V10m0-.6V4.4a1.5 1.5 0 0
      1 3 0V10m0-2.9a1.5 1.5 0 0 1 3 0V14c0 3.1-2.1 5.4-4.9 5.4S8.2 17.1 8.2 14v-2.4
      L6.4 9.8a1.4 1.4 0 0 0-2 2l2.6 3.1"/></svg>`;
...
// Which column the table is sorted by, and which way. Names read forwards,
// everything else backwards: nobody opens a squad list wanting the fewest
// appearances first.
const SORTKEY = { nm: 0, ap: 1, g: 2, st: 3, gk: 4, ss: 6 };
const seasonKey = c => c.slice(1) + (c[0] === 'p' ? '1' : '0');

function squadSort(sq, rows, by, dir) {
  const k = SORTKEY[by];
  // By surname: psmf.cz writes "Jméno Příjmení", and a squad list read by first
  // name is a list nobody scans.
  const surname = n => n.slice(n.lastIndexOf(' ') + 1) + ' ' + n;
  const val = r => by === 'nm' ? surname(sq.nm[r[0]]) : by === 'ss' ? seasonKey(r[6]) : r[k];
  return [...rows].sort((a, b) => {
    const x = val(a), y = val(b);
    const c = typeof x === 'string' ? x.localeCompare(y, 'cs') : x - y;
    // Appearances break every tie, so equal counts do not come out shuffled.
    return (c || a[1] - b[1]) * dir;
  });
}

function squadBlock(sq, name, from, to, by, dir) {
  const codes = squadCodes(sq, from, to);
  const t = squadSum(sq, codes);
  const head = (k, label, title) =>
    `<th${title ? ` title="${title}"` : ''}><button type="button" class="sortby"
      data-k="${k}"${by === k ? ' aria-sort="1"' : ''}>${label}${
      by === k ? (dir < 0 ? ' &darr;' : ' &uarr;') : ''}</button></th>`;
  // Keeping goal and the armband are marks rather than columns: each is one or
  // two people for years at a time, and a column of blanks says less than a
  // glove beside a name. Half a player's matches in goal makes him the keeper;
  ...
  const rows = squadSort(sq, t.p, by, dir).map(([i, ap, g, best, gk, cap, from, to]) => `<tr>
    <td><div class="cell"><span class="oname">${esc(sq.nm[i])}</span>${
      gk >= 5 && gk * 2 >= ap ? GLOVE : ''}${
      cap >= 5 && cap * 5 >= ap ? '<span class="capt" title="kapitán">C</span>' : ''}</div></td>
    <td class="num">${ap}</td><td class="num">${g || ''}</td><td class="num">${best || ''}</td>
    <td class="num">${gk || ''}</td>
    <td class="dim"><button type="button" class="span" data-a="${from}" data-b="${to}"
      title="Zúžit na sezony tohoto hráče">${esc(from === to ? seasonName(from)
      : seasonName(from) + ' – ' + seasonName(to))}</button></td></tr>`).join('');
  const ex = (x, what) => x
    ...
    histFor(t).then(data => {
      ...
      const apply = () => {
        document.getElementById('sqbox').innerHTML =
          squadBlock(data.sq, t.name, from, to, by, dir);
        ...
        // A player's first and last season are the range you want when you are
        // looking at him, so the cell that names them sets it.
        document.querySelectorAll('#sqbox .span').forEach(b => {
          b.addEventListener('click', () => {
            from = b.dataset.a; to = b.dataset.b; apply();
          });
        });
        document.querySelectorAll('#sqbox .sortby').forEach(b => {
          b.addEventListener('click', () => {
            // Same column again turns it round; a new one starts the way that
            // column is worth reading -- names up, counts down.
            if (b.dataset.k === by) dir = -dir;
            else { by = b.dataset.k; dir = by === 'nm' ? 1 : -1; }
            apply();
          });
        });
      };
